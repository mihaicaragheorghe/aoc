...
func solveA() int {
	...
	k, err := walk(maze, Directions.Up, start, path, 0)
	if err != nil {
		panic(err)
	}
	return k
...
func walk(maze [][]byte, dir Direction, curr Point, path map[Point]Direction, k int) (int, error) {
	...
	val, ok := path[curr]
	if ok && val == dir {
		return k, fmt.Errorf("infinite loop at count %d", k)
	} else if !ok {
		k++
	}
	path[curr] = dir
